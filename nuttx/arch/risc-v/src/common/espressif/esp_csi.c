...
/* Register-level capture probe.  Off by default: it costs ~1.2 s inside the
 * first capture start and floods the console.  Set to 1 when the camera stops
 * delivering frames -- it reports the host, bridge and DW-GDMA state, which is
 * what distinguishes "no data left the sensor" from "DMA mis-armed".
 */

#define ESP_CSI_CAPTURE_DIAG 0
...
/* TEMP-CAMERA-DIAG: single register snapshot of the whole MIPI receive path.
 *
 * Uses the real HAL device pointers, so the offsets cannot drift from the
 * hardware definitions.  The host has no int_raw register: int_st_main is
 * read-clear, so each call reports the errors latched since the previous one.
 */

#if ESP_CSI_CAPTURE_DIAG

static void esp_csi_diag(FAR const char *stage)
{
  FAR csi_host_dev_t *host = MIPI_CSI_HOST_LL_GET_HW(0);
  FAR csi_brg_dev_t  *brg  = MIPI_CSI_BRG_LL_GET_HW(0);
  int i;

  _info("CAMDIAG[%s] host ver=%08" PRIx32 " n_lanes=%u resetn=%u shz=%u "
        "rstz=%u rx=%08" PRIx32 " stop=%08" PRIx32 " cal=%08" PRIx32 "\n",
        stage, (uint32_t)host->version.val, (unsigned)host->n_lanes.n_lanes,
        (unsigned)host->csi2_resetn.csi2_resetn,
        (unsigned)host->phy_shutdownz.phy_shutdownz,
        (unsigned)host->dphy_rstz.dphy_rstz,
        (uint32_t)host->phy_rx.val, (uint32_t)host->phy_stopstate.val,
        (uint32_t)host->phy_cal.val);

  _info("CAMDIAG[%s] host st_main=%08" PRIx32 " msk_pkt=%08" PRIx32
        " msk_phy=%08" PRIx32 " msk_crc=%08" PRIx32 "\n",
        stage, (uint32_t)host->int_st_main.val,
        (uint32_t)host->int_msk_pkt_fatal.val,
        (uint32_t)host->int_msk_phy_fatal.val,
        (uint32_t)host->int_msk_crc_frame_fatal.val);

  _info("CAMDIAG[%s] brg  clk_en=%u csi_en=%u hctrl=%08" PRIx32
        " raw=%08" PRIx32 " ena=%08" PRIx32 " dtype=%08" PRIx32
        " frame=%08" PRIx32 " flow=%08" PRIx32 " req=%08" PRIx32
        " blk=%08" PRIx32 "\n",
        stage, (unsigned)brg->clk_en.clk_en, (unsigned)brg->csi_en.csi_brg_en,
        (uint32_t)brg->host_ctrl.val, (uint32_t)brg->int_raw.val,
        (uint32_t)brg->int_ena.val, (uint32_t)brg->data_type_cfg.val,
        (uint32_t)brg->frame_cfg.val, (uint32_t)brg->buf_flow_ctl.val,
        (uint32_t)brg->dma_req_cfg.val, (uint32_t)brg->dmablk_size.val);

  /* NOTE: do NOT CPU-read the bridge data FIFO at MIPI_CSI_BRG_MEM_BASE.  That
   * address is only reachable by DW-GDMA; a direct load faults the bus and
   * takes the whole system down (observed: task dump, board unreachable).
   */

  _info("CAMDIAG[%s] dma  gcfg=%08" PRIx32 " chen0=%08" PRIx32
        " int_st0=%08" PRIx32 "\n", stage,
        (uint32_t)DW_GDMA.cfg0.val, (uint32_t)DW_GDMA.chen0.val,
        (uint32_t)DW_GDMA.int_st0.val);

  for (i = 0; i < 2; i++)
    {
      _info("CAMDIAG[%s] dma%d sar=%08" PRIx32 " dar=%08" PRIx32
            " ts=%08" PRIx32 " ctl0=%08" PRIx32 " cfg0=%08" PRIx32
            " st=%08" PRIx32 " sstat=%08" PRIx32 " dstat=%08" PRIx32 "\n",
            stage, i,
            (uint32_t)DW_GDMA.ch[i].sar0.val,
            (uint32_t)DW_GDMA.ch[i].dar0.val,
            (uint32_t)DW_GDMA.ch[i].block_ts0.val,
            (uint32_t)DW_GDMA.ch[i].ctl0.val,
            (uint32_t)DW_GDMA.ch[i].cfg0.val,
            (uint32_t)DW_GDMA.ch[i].status0.val,
            (uint32_t)DW_GDMA.ch[i].sstat0.val,
            (uint32_t)DW_GDMA.ch[i].dstat0.val);
    }
}

/* TEMP-CAMERA-DIAG: sample the receive path over time.  A static dar means the
 * bridge never produced a DMA request, which is the signature of "bridge
 * receives no valid frame" as opposed to "DMA mis-armed".
 */

static struct work_s g_csi_diag_work;

static void esp_csi_diag_series(FAR void *arg)
{
  FAR csi_brg_dev_t *brg = MIPI_CSI_BRG_LL_GET_HW(0);
  uint32_t dar0;
  uint32_t dar3;
  ...
  esp_csi_diag("t0");
  dar0 = DW_GDMA.ch[0].dar0.val;

  for (i = 1; i <= 3; i++)
    {
      up_mdelay(400);
      esp_csi_diag(i == 1 ? "t1" : (i == 2 ? "t2" : "t3"));
    }

  dar3 = DW_GDMA.ch[0].dar0.val;

  /* One-line verdict: ch0 dar only moves when the bridge actually feeds the
   * channel, so this is the single unambiguous "frames are arriving" signal.
   */

  _info("CAMDIAG[RESULT] ch0 dar %08" PRIx32 " -> %08" PRIx32 " : %s"
        " | brg int_raw=%08" PRIx32 " csi_en=%u buf_depth=%u\n",
        dar0, dar3,
        dar0 != dar3 ? "MOVING - FRAMES ARRIVING" : "STATIC - no frames",
        (uint32_t)brg->int_raw.val, (unsigned)brg->csi_en.csi_brg_en,
        (unsigned)brg->buf_flow_ctl.csi_buf_depth);

}

#endif /* ESP_CSI_CAPTURE_DIAG */
                                      ...
                                      FAR void *arg)
{
  ...
  esp_err_t csi_start_err = esp_cam_ctlr_start(priv->ctlr);

  /* Report the controller-start result either way.  s_ctlr_csi_start() runs a
   * fixed sequence -- cache msync(M2C), dw_gdma_channel_config_transfer(),
   * dw_gdma_channel_enable_ctrl(), mipi_csi_brg_ll_enable() -- and every step is
   * an ESP_RETURN_ON_ERROR, so a zero here means the DMA really is configured
   * against the bridge and the bridge really is enabled.  That is what
   * separates "capture never started" from "started but no data arrives from
   * the MIPI receive side", which is otherwise indistinguishable from the
   * outside (both show an armed channel and a static frame buffer).
   */

  _info("CSI: ctlr start rc=%d (%s)\n", (int)csi_start_err,
        csi_start_err == ESP_OK ? "DMA+bridge enabled" : "FAILED");

  if (csi_start_err != ESP_OK)
    {
      _err("ERROR: esp_cam_ctlr_start failed: %d\n", (int)csi_start_err);
      priv->callback = NULL;
      priv->cbarg    = NULL;
      return -EIO;
    }
...
#if ESP_CSI_CAPTURE_DIAG
  /* The sensor is still in sleep mode at this point (the SC2336 stream-on
   * happens later in the VIDIOC_S_FMT/REQBUFS chain), so defer the sampling
   * by 3 s to land inside the streaming window.
   */

  work_queue(LPWORK, &g_csi_diag_work, esp_csi_diag_series, NULL,
             MSEC2TICK(3000));
#endif
