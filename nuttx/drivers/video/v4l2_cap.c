  ...
  switch (fmt->fmt.pix.pixelformat)
    {
      ...
      /* Raw Bayer formats: a MIPI-CSI sensor without an ISP in the pipeline
       * delivers the mosaic straight from the sensor (e.g. the SC2336 on the
       * ESP32-P4X board produces V4L2_PIX_FMT_SBGGR10).  These pass through
       * the imgdata lower half untouched, so accept them here.
       */
