...
/****************************************************************************
 * Name: netinit_wait_for_link
 *
 * Description:
 *   Wait until the interface reports carrier (IFF_RUNNING) or the configured
 *   timeout (CONFIG_NETINIT_DHCPC_LINK_WAIT_MS) expires.
 *
 *   netlib_ifup() only *starts* the PHY state machine; the link itself is
 *   negotiated asynchronously a moment later.  If DHCP is started before the
 *   link is up the first DISCOVER is transmitted into a dead link, is
 *   silently lost, and the board finishes booting with 0.0.0.0.
 *
 *   IFF_RUNNING is set only by netdev_carrier_on() and cleared only by
 *   netdev_carrier_off(), so it is a true carrier indication (it is NOT set
 *   merely by bringing the interface up).
 *
 ****************************************************************************/

#ifdef CONFIG_NETUTILS_DHCPC
static void netinit_wait_for_link(void)
{
  ...
  if (CONFIG_NETINIT_DHCPC_LINK_WAIT_MS <= 0)
    {
      return;
    }
  ...
  for (remaining = CONFIG_NETINIT_DHCPC_LINK_WAIT_MS;
       remaining > 0; remaining -= 100)
    {
      ...
      if (ioctl(sd, SIOCGIFFLAGS, (unsigned long)&ifr) >= 0 &&
        ...
        {
          ninfo("Carrier up after %d ms\n",
                CONFIG_NETINIT_DHCPC_LINK_WAIT_MS - remaining);
          break;
        }
...
static void netinit_net_bringup(void)
{
  ...
  if (g_use_dhcpc)
    {
      /* The link is negotiated asynchronously after ifup; starting DHCP
       * immediately races the PHY and the DISCOVER is lost.  Wait for
       * carrier first (bounded by CONFIG_NETINIT_DHCPC_LINK_WAIT_MS).
       */

      netinit_wait_for_link();
