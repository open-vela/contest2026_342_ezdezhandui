...
int network_set_dns(const char* primary, const char* secondary)
{
    ...
    /* 文件只是后备来源：真正生效的是解析器内部的 nameserver 列表。
     * 不调用 dns_add_nameserver() 时，即便 resolv.conf 写对了，
     * gethostbyname()/getaddrinfo() 依然查不到 —— 表现为
     * "nslookup -> getaddrinfo failed: 1"。
     */
...
#ifdef CONFIG_NETDB_DNSCLIENT
    {
        ...
        dns.sin_addr.s_addr = inet_addr(primary);
        ret = dns_add_nameserver((FAR const struct sockaddr *)&dns,
                                 sizeof(dns));
        if (ret < 0)
          {
            syslog(LOG_WARNING, "[%s] dns_add_nameserver(%s) failed: %d\n",
                TAG, primary, ret);
          }
        else
          {
            syslog(LOG_INFO, "[%s] resolver nameserver <- %s\n", TAG,
                primary);
          }

        if (secondary && secondary[0] != '\0')
          {
            dns.sin_addr.s_addr = inet_addr(secondary);
            ret = dns_add_nameserver((FAR const struct sockaddr *)&dns,
                                     sizeof(dns));
            if (ret >= 0)
              {
