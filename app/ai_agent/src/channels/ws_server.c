...
static void* client_thread(void* arg)
{
    ...
    if (!peek_buf) {
        syslog(LOG_ERR, "[%s] OOM: no buffer for fd=%d\n", TAG, fd);
        close(fd);
        return NULL;
    }
    int peek_total = 0;
    while (peek_total < 2048 - 1) {
        int n = recv(fd, peek_buf + peek_total,
            2048 - 1 - peek_total, 0);
        if (n <= 0) {
            /* Silent before: a client that connects and sends nothing (or a
             * peer that resets) looked exactly like a broken server.  Log it
             * so the console tells the two apart. */
            syslog(LOG_WARNING, "[%s] recv failed: fd=%d n=%d errno=%d\n",
                TAG, fd, n, errno);
            free(peek_buf);
            close(fd);
            return NULL;
...
{
    (void)arg;
    syslog(LOG_INFO, "[%s] accept loop running (listen_fd=%d)\n", TAG,
        s_listen_fd);
    while (s_running) {
        ...
        if (pr == 0)
            continue;
        if (pr < 0) {
            syslog(LOG_WARNING, "[%s] poll failed errno=%d\n", TAG, errno);
            continue;
        }
        ...
        syslog(LOG_INFO, "[%s] client connected: fd=%d\n", TAG, cfd);

        client_arg_t* ca = malloc(sizeof(client_arg_t));
        if (!ca) {
            syslog(LOG_ERR, "[%s] OOM for client_arg fd=%d\n", TAG, cfd);
            close(cfd);
            continue;
        }
        ca->fd = cfd;
        ...
        if (pthread_create(&tid, &attr, client_thread, ca) != 0) {
            syslog(LOG_ERR, "[%s] pthread_create failed for fd=%d errno=%d\n",
                TAG, cfd, errno);
            free(ca);
            close(cfd);
        }
