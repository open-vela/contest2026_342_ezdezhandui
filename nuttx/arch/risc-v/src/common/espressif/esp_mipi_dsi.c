...
#define ESP_MIPI_DSI_DMA_CHAN     1
...
/* Bring-up trace for the DW-GDMA streaming path.
 *
 * Set to 0 (or delete together with esp_mipi_dsi_dma_trace()) once continuous
 * FB streaming is verified on hardware.  While enabled it reports, from the
 * firmware itself, whether the channel was ever armed, whether the LLI
 * terminator was reached and whether TFR_DONE keeps recurring -- three facts
 * that cannot be distinguished from the outside (a breakpoint on the ISR plus
 * a register peek cannot tell "never armed" from "completed and stopped").
 */

#define ESP_MIPI_DSI_DMA_TRACE 1

#if ESP_MIPI_DSI_DMA_TRACE
static volatile uint32_t g_dsi_dma_isr_count;
static volatile uint32_t g_dsi_dma_restart_count;

static void esp_mipi_dsi_dma_trace(FAR struct esp_mipi_dsi_priv_s *priv,
                                   FAR const char *tag)
{
  FAR dw_gdma_dev_t *dev = priv->dw_hal.dev;
  FAR volatile dmac_channel_reg_t *ch;
  ...
  ch = &dev->ch[ESP_MIPI_DSI_DMA_CHAN];

  syslog(LOG_INFO,
         "DSI-DMA[%s] fb=%p lli=%p nc=%p isr=%lu rst=%lu "
         "chen0=%08lx ist=%08lx cist=%08lx sar=%08lx dar=%08lx "
         "bts=%08lx ctl=%08lx llp=%08lx cfg=%08lx st=%08lx "
         "dstat=%08lx brg=%08lx\n",
         tag, priv->fb, priv->lli, priv->lli_nc,
         (unsigned long)g_dsi_dma_isr_count,
         (unsigned long)g_dsi_dma_restart_count,
         (unsigned long)dev->chen0.val,
         (unsigned long)dev->int_st0.val,
         (unsigned long)dev->common_int_st0.val,
         (unsigned long)ch->sar0.val,
         (unsigned long)ch->dar0.val,
         (unsigned long)ch->block_ts0.val,
         (unsigned long)ch->ctl0.val,
         (unsigned long)ch->llp0.val,
         (unsigned long)ch->cfg0.val,
         (unsigned long)ch->status0.val,
         (unsigned long)ch->dstat0.val,
         (unsigned long)mipi_dsi_brg_ll_get_interrupt_status(priv->hal.bridge));
}
#endif
                                          ...
                                          FAR void *arg)
{
  ...
  /* Clear only the bits that are actually set.
   *
   * Do NOT widen this to UINT32_MAX: int_clr0 is not a plain
   * write-1-to-clear register for every bit.  Writing all ones was measured
   * to wedge the channel outright -- chen0 stayed 0 with int_st0 bit0
   * permanently pending, which left the CPU spinning in this handler (the
   * board still answered ping but NSH never got the CPU again and the panel
   * never updated).
   */
...
static int esp_mipi_dsi_dma_setup(FAR struct esp_mipi_dsi_priv_s *priv)
{
  ...
  /* The CSI camera shares this DW-GDMA controller and is initialised before the
   * display (see esp32p4_bringup.c), so this driver must not blindly reset it.
   *
   * Both dw_gdma_ll_reset_register() and dw_gdma_hal_init() reset the *whole*
   * controller -- and dw_gdma_hal_init() does it unconditionally.  That wipes
   * the camera's channel configuration, so its capture can never start and the
   * next register access faults:
   *
   *   esp_csi_data_start_capture: ERROR: esp_cam_ctlr_start failed
   *   Store/AMO access fault, EPC in dw_gdma_channel_config_transfer,
   *   MTVAL = 0x50081100 (the DW-GDMA channel-0 register block)
   *
   * The controller's peripheral clock is the tell-tale: it is switched on by
   * the first user to acquire a group handle and gated off again by
   * dw_gdma_release_group_handle() when the last one releases.  A live bus
   * clock therefore means a sibling channel already owns controller state that
   * must survive, so only reset when we are the first user.
   *
   * Reordering the two initialisations (camera first) previously moved this
   * failure from the panel to the camera rather than removing it.
   */
  ...
  /* Stand up our own HAL context without the global reset that
   * dw_gdma_hal_init() performs.  The channel is configured from scratch
   * below, so it does not depend on inheriting a reset state.
   */

  priv->dw_hal.dev = DW_GDMA_LL_GET_HW(0);
  dw_gdma_ll_enable_controller(priv->dw_hal.dev, true);
  dw_gdma_ll_enable_intr_global(priv->dw_hal.dev, true);
  dev = priv->dw_hal.dev;
  ...
  dw_gdma_ll_channel_set_dst_outstanding_limit(dev, ESP_MIPI_DSI_DMA_CHAN,
                                               2);
  /* Narrow the channel's interrupt generation *and* propagation masks down to
   * the single event the ISR handles.
   *
   * Both registers reset to all-ones ("default: 1" for every event bit), so
   * OR-ing the wanted bit in is a no-op: the channel keeps generating and
   * propagating an interrupt for every completion event, including one per
   * bridge-FIFO burst (~860 per frame).  Measured on hardware before this
   * fix: 13433 ISR entries in 200 ms (~67 kHz) while only 23 transfers
   * actually completed, plus 25% ICMP packet loss from the interrupt load.
   *
   * Clear the masks first, then set only DMA_TFR_DONE.
   */

  dw_gdma_ll_channel_enable_intr_generation(dev, ESP_MIPI_DSI_DMA_CHAN,
                                            UINT32_MAX, false);
  dw_gdma_ll_channel_enable_intr_propagation(dev, ESP_MIPI_DSI_DMA_CHAN,
                                             UINT32_MAX, false);
  ...
  if (priv->dma_cpuint < 0)
    {
      ...
      /* NOTE on the status register: pass the controller's aggregated channel
       * status register and this channel's bit, exactly like the HAL does.
       *
       *   esp_intr_alloc_intrstatus(ETS_DW_GDMA_INTR_SOURCE, isr_flags,
       *                             dw_gdma_ll_get_intr_status_reg(dev),
       *                             DW_GDMA_LL_CHANNEL_EVENT_MASK(chan_id), ...);
       *
       * int_st0 bit[chan] is that channel's "interrupt pending" flag, so with
       * it the handler runs only for its own channel.
       *
       * Registering without a status register instead enters this ISR for
       * *every* DW_GDMA interrupt source.  Measured on hardware: 13237 ISR
       * entries in 200 ms (~66 kHz) while the DSI channel completed only 23
       * transfers in the same window -- a storm that starved the CPU enough to
       * lose 25% of ICMP replies.
       *
       * (An earlier attempt at this was wrongly blamed for a frozen panel:
       * that panel was already frozen by the arm-before-DPI ordering bug in
       * esp_mipi_dsi_video_start(), which is fixed there.)
       */
...
int esp_mipi_dsi_video_start(void)
{
  ...
  /* Arm the DW-GDMA only *after* the DPI pixel stream is running.
   *
   * Channel 0 has the bridge FIFO (MIPI_DSI_BRG_MEM_BASE) as its destination
   * with a hardware handshake on it, and the bridge only pops that FIFO while
   * its DPI output is enabled.  Arming first pushes pixels into a FIFO that
   * nothing drains, so the transfer stalls before it can raise TFR_DONE -- and
   * because the LLI terminates itself (llp == 0x1) the channel then stays
   * disabled forever, which is exactly the "one frame, then frozen panel"
   * behaviour.
   */

#if ESP_MIPI_DSI_DMA_TRACE
  esp_mipi_dsi_dma_trace(priv, "before-arm");
#endif
...
#if ESP_MIPI_DSI_DMA_TRACE
  esp_mipi_dsi_dma_trace(priv, "after-arm");
  up_mdelay(200);
  esp_mipi_dsi_dma_trace(priv, "t200ms");
#endif
