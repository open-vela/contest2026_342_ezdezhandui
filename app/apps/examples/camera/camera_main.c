...
/* Capture is exercised in the sensor's native RAW10 Bayer format (see the
 * V4L2_PIX_FMT_SBGGR10 request in camera_main), so the frame buffer has to be
 * sized for that rather than for QVGA RGB565.  The CSI driver rejects anything
 * smaller than h_res * v_res * 10 / 8, which for the 1920x1080 SC2336 is
 * 2592000 bytes -- 17x IMAGE_RGB_SIZE.  Handing it IMAGE_RGB_SIZE made
 * esp_csi_data_set_buf() fail and the follow-up error path touch the DW-GDMA
 * registers with the controller clock off, ending in a store access fault at
 * 0x50081100 (the DW-GDMA channel-0 block).
 */

#define IMAGE_RAW10_SIZE \
  ((size_t)CONFIG_ESPRESSIF_MIPI_CSI_H_RES * \
   (size_t)CONFIG_ESPRESSIF_MIPI_CSI_V_RES * 10u / 8u)
  ...
  for (cnt = 0; cnt < buffernum; cnt++)
    {
       ...
       *
       * The buffer must be aligned to the *cache line* of the SoC, not just to
       * 32 bytes: the CSI driver calls esp_cache_msync() on it for the M2C
       * invalidate before a capture, and that rejects any start address that
       * is not a multiple of the cache line size (64 bytes on ESP32-P4).
       *
       * With 32-byte alignment the allocation could land on an address like
       * 0x483a4da0 -- aligned for a 32-byte rule but not for 64 -- and the
       * capture then failed with:
       *   esp_cache_msync: start address ... not aligned with cache line size
       *   CSI: s_ctlr_csi_start(): failed to sync(M2C) trans buffer
...
int main(int argc, FAR char *argv[])
{
  ...
  /* openvela/ESP32-P4: the board already registers the capture device from
   * esp_bringup(), so capture_initialize() is expected to fail here.  Treat
   * that as non-fatal and use the device the board created.
   */

  ret = capture_initialize(CAMERA_DEV_PATH);
  if (ret != 0)
    {
      printf("NOTE: capture_initialize() rc=%d errno=%d (device already "
             "registered by the board, continuing)\n", ret, errno);
    }
  ...
  if (capture_num != 0)
    {
      ...
      /* openvela/ESP32-P4: the SC2336 driver exposes no STILL_CAPTURE /
       * JPEG path (there is no ISP in the pipeline yet), so skip it and use
       * the video-capture path only.
       */

      printf("NOTE: skipping STILL_CAPTURE/JPEG (not supported by SC2336)\n");
    }
  ...
  /* openvela/ESP32-P4: request the format the SC2336 actually produces.
   * There is no ISP converting the Bayer mosaic to RGB565 yet, so the
   * capture is exercised in the sensor's native RAW10 Bayer format.
   */

  ret = camera_prepare(v_fd, V4L2_BUF_TYPE_VIDEO_CAPTURE,
                       V4L2_BUF_MODE_RING, V4L2_PIX_FMT_SBGGR10,
                       CONFIG_ESPRESSIF_MIPI_CSI_H_RES,
                       CONFIG_ESPRESSIF_MIPI_CSI_V_RES,
                       &buffers_video, VIDEO_BUFNUM, IMAGE_RAW10_SIZE);
  if (ret != OK)
    {
      goto exit_this_app;
    }
