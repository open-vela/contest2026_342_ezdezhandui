                                  ...
                                  size_t len)
{
  /* ⚠️ 这里的 `= { 0 }` 是必须的, 不是风格问题。
   *
   * struct mipi_dsi_msg 有两个发送路径用不到的字段 rx_len / rx_buf。
   * 只逐字段赋值时它们保持**栈上残留值**, 而 DSI host 恰恰靠这两个字段
   * 判断本次传输是读还是写:
   *
   *   esp_mipi_dsi.c:899
   *     is_read = (msg->rx_len > 0 && msg->rx_buf != NULL);
   *
   * 栈残留值一旦满足条件, 一条本该 fire-and-forget 的 DCS **写** 会被当成
   * **读** 发出去 -> esp_mipi_dsi_read_short() -> 等 BTA 应答 -> 无人应答
   * -> -110 (ETIMEDOUT)。
   *
   * 2026-09-14 真机实测 (EK79007 @ ESP32-P4X): 面板初始化第一条 DCS 0xb2
   * 就报
   *     esp_mipi_dsi_read_short: read FIFO empty timeout
   *     ERROR: EK79007 DCS 0xb2 failed: -110 (1/9)
   * 现象极像"面板没接/没供电", 实际根因是这里的未初始化变量。
   * 本文件 8 处 msg 声明现已全部改为 `= { 0 }`。
   */
