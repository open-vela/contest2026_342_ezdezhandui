...
/* openvela: job table is heap-allocated (heap lives in PSRAM) instead of a
 * static .bss array, freeing ~11.4KB of SRAM for code space. */
static cron_job_t* s_jobs;
...
/* Lazily allocate the job table; every public entry point calls this before
 * touching s_jobs[]. */
static int cron_ensure_jobs(void)
{
    if (s_jobs != NULL) {
        return OK;
    }

    s_jobs = (cron_job_t*)calloc(MAX_CRON_JOBS, sizeof(cron_job_t));
    if (s_jobs == NULL) {
        syslog(LOG_ERR, "[%s] Failed to allocate job table (%d jobs)\n",
            TAG, MAX_CRON_JOBS);
        return -ENOMEM;
    }
